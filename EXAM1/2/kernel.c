...
int khop (char* s)   // s points to the segment name in Umode
{
	int length = 0;
	char segment[64], *cp = segment;
	u16 uL;

	printf ("inside kernel to execute the hop...\n");

	// get the segment from user space with a length limit of 64
	while ((*cp++ = get_byte (running -> uss, s++)) && length++ < 64);

	uL = strtoul (segment, 0, 0);			// convert the string to a u16

	copyImage (running -> uss, uL, 32*1024);	// copy the whole 32k words
	running -> uss = uL;				// change segments

	// restore uDS, uES, uCS, AX 
	put_word (uL, uL, running -> usp);			// uDS = segment
	put_word (uL, uL, ((running -> usp) + 2));		// uES = segment
	put_word (0, uL, ((running -> usp) + (2 * 8)));		// uax = 0
	put_word (uL, uL, ((running -> usp) + (2 * 10)));	// uCS = segment
