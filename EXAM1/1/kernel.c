...
{
	int i, length = 0, slength;
	char filename[64], *cp = filename, *token, *command, copy[64], *s = y;
	...
	// get the command from y
	strcpy (copy, filename);		// save a copy of the command line
	slength = strlen (copy);		// get the string length of the command line
	token = strtok (filename, " ");		// retrieve the command
	strcpy (command, token);		// copy the command into command
	strcpy (filename, "/bin/");		// overwrite filename with "/bin/"
	strcat (filename, command);		// concatenate /bin/ with the command
	...
	if (((slength + 1) % 2) == 0)	// if the string length is odd
	{
		// make it even
		printf ("string length is odd... making it even\n");
		slength++;
	}

	for (i = 1; i < 12; i++)
		put_word (0, segment, -2*i);
	running -> usp = -24 - (2 + slength);
	
	put_word (copy, segment, -slength);			// save the command line at the high end of stack
	put_word (s, segment, -slength-2);			// save the s pointer right after the command line
	put_word (segment, segment, (-slength-2)-(2*12));	// saved uDS = segment
	put_word (segment, segment, (-slength-2)-(2*11));	// saved uES = segment
	put_word (segment, segment, (-slength-2)-(2*2));	// uCS = segment; uPC = 0
	put_word (0x0200, segment, (-slength-2)-(2*1));		// Umode flag = 0x0200
