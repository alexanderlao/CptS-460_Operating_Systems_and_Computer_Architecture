...
char *cmd[]={"getpid", "ps", "chname", "kfork", "switch", "wait", "exit", "fork", "exec",
             "pipe", "read", "write", "close", "pfd", 0};
...
int show_menu()
{
   ...
   printf("*  pipe  read  write  close  pfd                      *\n");
   printf("*******************************************************\n");
}
...
int uread()
{
    char buf[64], s[16], t[16];
    int ufd, n, nbytes;

    printf ("enter a fd to read from: ");
    gets (s);
    ufd = atoi (s);

    printf ("enter a number of bytes to read: ");
    gets (t);
    nbytes = atoi (t);

    n = syscall (31, ufd, buf, nbytes);

    if (n >= 0)
    {
	printf ("PROC %d read %d bytes from the pipe!\n", getpid (), n);
	buf[n] = 0;
	printf ("read text = %s\n", buf);
    }
    else
    {
	printf ("Reading from the pipe failed!\n");
    }
}
...
int uwrite()
{
    ...
    printf ("passing string = %s to write\n", buf);

    n = syscall (32, ufd, buf, nbytes);

    if (n >= 0)
	printf ("PROC %d wrote %d bytes to the pipe!\n", getpid (), n);
    else
