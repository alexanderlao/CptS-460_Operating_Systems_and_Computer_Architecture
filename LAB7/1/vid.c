...
#define LINE_WIDTH	80		// number of characters on a line
#define SCR_LINES	25		// number of lines on a screen
...
u16 vid_mask = 0x3FFF;	//make=Video RAM size - 1
...
int vid_init() {	// initialize org=0 (row, column) = (0, 0)
	int i, w;
	org = row = column = 0;
...
int scroll() {	// scroll UP one line
	...
	if (offset <= vid_mask) {	// offset still within VRAM area
		org += 2 * LINE_WIDTH;	// just advance org by ONE line
			...
			// copying current rows 1-24 to BASE, then reset org to 0
		for (i = 0; i < 24 * 80; i++) {
