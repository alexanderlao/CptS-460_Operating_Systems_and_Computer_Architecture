// serial.c file for SERIAL LAB ASSIGNMENT
/**************** CONSTANTS ***********************/
#define BUFLEN      80
#define INBUFLEN    80
#define OUTBUFLEN   80
...
int ksin(char *y)
{
  ...
  len = sgetline(&stty[0], inLine);

  do
  {
	put_byte(inLine[i++], running -> uss, y++);
  }while(inLine[i] != '\0');
